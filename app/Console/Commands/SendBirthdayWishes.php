...
class SendBirthdayWishes extends Command
{
    ...
    /**
     * SMTP connection details come from .env (config/mail.php) - not the database.
     * Only the visible sender identity is taken from Email Settings.
     */
    ...
    private function sendBirthdayEmail($employee, $template)
    {
        ...
        try {
            ...
            // Cron discards console output, so record the reason where it survives.
            \Illuminate\Support\Facades\Log::error('Birthday email failed', [
    ...
    private function sendBirthdaySms($employee, $template, $sms)
    {
        ...
        try {
            ...
            // Cron discards console output, so record the reason where it survives.
            \Illuminate\Support\Facades\Log::error('Birthday SMS failed', [
                'employee' => $employee->full_name,
                'to'       => $recipient,
                'reason'   => $e->getMessage(),
            ]);
